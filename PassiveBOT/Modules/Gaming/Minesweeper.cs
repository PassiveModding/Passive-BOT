...
namespace PassiveBOT.Modules.Gaming
{
    ...
    /// <summary>
    /// Minesweeper related content for the games module
    /// </summary>
    public partial class Games
    {
        ...
        public async Task<bool> IsGameAcceptable(int width, int height, int mine_count)
        {
            ...
            {
                await SimpleEmbedAsync("Mine count must not be greater than amount of squares on board.");
                return false;
            }

            if (width <= 0 || height <= 0)
            {
                await SimpleEmbedAsync("Width and height must be greater than 0");
                return false;
            }

            if (mine_count <= 0)
            {
                await SimpleEmbedAsync("What fun would minesweeper be if there were no bombs.");
                return false;
            }
        ...
        public async Task MinefieldGameAsync(int width = 10, int height = 10, int mine_count = 10, bool quick = false)
        {
            try
            {
                ...
                {
                    int mine_x = rnd.Next(width);
                    int mine_y = rnd.Next(height);
                ...
                {
                    await SimpleEmbedAsync("Sorry, a minefield this big will exceed discords message length limit, please try a smaller board size.");
                    return;
                }
