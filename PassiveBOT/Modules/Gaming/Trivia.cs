...
namespace PassiveBOT.Modules.Gaming
{
    ...
    /// <summary>
    /// Trivia module and logic for games
    /// </summary>
    public partial class Games
    {
        ...
        public async Task TriviaSettingsAsync()
        {
            ...
            await SimpleEmbedAsync($"**Trivia Command:**\n" +
        ...
        public async Task TriviaAsync(int questions = 10, string categoryName = null, TriviaDifficulty difficulty = TriviaDifficulty.none, TriviaType type = TriviaType.none)
        {
            ...
            await SimpleEmbedAsync("Unable to get trivia questions");
        }

        public async Task NextQuestionAsync(Trivia trivia, int currentQuestionIndex, int totalCorrect, string resString = "")
        {
            Trivia.Result question = null;
            try
            {
            ...
            if (question != null)
            {
                ...
                var rnd = Context.Provider.GetRequiredService<Random>();
                possibleAnswers = possibleAnswers.OrderByDescending(x => rnd.Next()).ToList();
            ...
            await SimpleEmbedAsync($"Trivia Complete! You got {totalCorrect}/{trivia.results.Count} correct\n" + 
        ...
        private async Task<Trivia> NewTriviaAsync(ulong guildId, int questions, TriviaDifficulty difficulty, TriviaType type, TriviaCategories.TriviaCategory category = null)
        {
            try
            {
                if (questions <= 0)
                ...
                var res = await Client.GetStringAsync(baseUrl);
                var resObject = JsonConvert.DeserializeObject<Trivia>(res);

                if (resObject.response_code == 0)
                {
                    resObject.results = resObject.results.Select(x => new Trivia.Result
                                                                          {
