    ...
    public class NSFW : Base
    {
        ...
        public async Task BRedditNSFWAsync(string subreddit = null)
        {
            ...
            else
            {
                ...
                RedditModels.SubReddits.Add(new RedditModels.SubReddit { Title = subreddit, LastUpdate = DateTime.UtcNow, Posts = posts, Hits = 0, NSFW = true });
            }
            ...
            var msg = new PaginatedMessage { Title = $"{subreddit} Images", Pages = pages, Color = new Color(114, 137, 218) };
        ...
        public async Task RedditNSFWAsync(string subreddit = null)
        {
            ...
            else
            {
                ...
                posts = await sub.GetTop(FromTime.Week, 150).Where(x => RedditHelper.IsImage(x.Url.ToString()).IsImage).ToList();
                RedditModels.SubReddits.RemoveAll(x => string.Equals(x.Title, subreddit, StringComparison.CurrentCultureIgnoreCase));
                RedditModels.SubReddits.Add(new RedditModels.SubReddit { Title = subreddit, LastUpdate = DateTime.UtcNow, Posts = posts, NSFW = true });
            }
