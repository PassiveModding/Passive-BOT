...
namespace PassiveBOT.Commands
{
    ...
    public class Translate : ModuleBase
    {
        ...
        public async Task TranslateCmd(string language, [Remainder] string message)
        {
            ...
            var url = "https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=" + language + "&dt=t&q=" + Uri.EscapeDataString(message);
            ...
            dynamic file = JsonConvert.DeserializeObject(File.ReadAllText(Path.Combine(AppContext.BaseDirectory, $"{Context.Message.Id}.txt")));
        ...
        public async Task Premium([Remainder]string key = null)
        {
            if (key == null)
            {
                await ReplyAsync("Please upgrade to use the premium version of the Translation API to use the premium translation commands, and convert between 80+ languages with ease\n" +
        ...
        public async Task Translate2(string target, [Remainder] string message)
        {
            ...
            {
                await ReplyAsync("Please upgrade to use the premium version of the Translation API to use this command, and convert between 80+ languages with ease\n" +
                                 "Purchase: http://rocketr.net/sellers/passivemodding");
                return;
            }

            if (GuildConfig.Load(Context.Guild.Id).TimeOffset < DateTime.UtcNow && GuildConfig.Load(Context.Guild.Id).Premium)
            {
                var g = GuildConfig.GetServer(Context.Guild);
                g.Premium = false;
                g.Characters = 0;
            ...
            try
            {
                ...
                embed.WithFooter(x =>
                {
                    x.Text = $"{(GuildConfig.Load(Context.Guild.Id).TimeOffset - DateTime.UtcNow):dd\\ \\D\\a\\y\\s\\ hh\\ \\H\\o\\u\\r\\s} Left || [{g.Characters}/100000]";
                });
                GuildConfig.SaveServer(g, Context.Guild);
                await ReplyAsync("", false, embed.Build());
            }
